...
# Run the simulation in a sliced way
# and produce Component 4: the results table
slice_num = 0
for slice in tqdm(grid.iter_slices(10000), leave=True, total=len(grid) // 10000 + 1):
    # init output lists
    estimates = []
    pvalues = []
    singulars = []
    ...
    for row in tqdm(slice.iter_rows(named=True), total=len(slice), leave=False):
        df = generate_data(
    ...
    results_slice.write_parquet(f"output/{slice_num:03}.parquet")
    slice_num += 1
