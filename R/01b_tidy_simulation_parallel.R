...
run_chunk <- function(chunk_id, chunk_size) {
  ...
  chunk_table <- bind_rows(chunk_list) |> 
    mutate(converged = as.logical(converged))
  
  # then write to output directory as parquet
  out_name <- paste0("output/", str_pad(chunk_id, 6, pad = 0), ".parquet")
...
# NB: you don't NEED to chunk the grid, you could also run the following 
# to generate a results list in 1 go in memory
# results_list <- pblapply(1:nrow(grid), run_simulation, cl = clus)
